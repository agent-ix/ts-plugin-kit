...
function deploy() {
    ...
    exec(`DEV_PATH=${DEV_PATH} WATCH_POLLING=${WATCH_POLLING} envsubst < k8s/storybook/deployment.yaml.template | kubectl apply -f -`);

    exec(`kubectl rollout restart deployment ${DEPLOYMENT_NAME}`);

    console.log(`✅ Dev deployment complete. Code changes will hot-reload at ${APP_URL}`);
    ...
    if (WATCH_POLLING === 'true') {
        ...
        console.log('💡 If changes aren\'t detected, try: WATCH_POLLING=true pnpm run storybook:deploy');
    }
}
