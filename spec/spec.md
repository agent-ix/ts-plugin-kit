...
surface is synchronous with `git` (via `execFileSync`) as its only side effect; the
discovery surface (`search.ts`) is asynchronous and performs all network I/O through
the injectable `HttpFetcher` (default: the Node global `fetch`, so no dependency is
added).
